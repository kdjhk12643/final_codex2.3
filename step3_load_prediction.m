...
fprintf("  步骤 3.6 运行 LSTM/BP 公平输入对比实验...\n");
% 公平对比让 LSTM 与 BP 在同一特征组、同一预测时间点上比较，避免输入信息量差异被误读为模型能力差异。
predictionResult.fairModelComparison = runFairModelComparison( ...
    cfg, featureData, selectedFeatures, cfg.targetName, predictionResult.metricsLSTM);
...
fprintf("  步骤 3.9 基于 LSTM 预测结果推算子系统负荷曲线...\n");
...
fprintf("  步骤 3.10 生成子系统置信需求和情景需求表...\n");
...
fairComparisonFile = fullfile(cfg.tableDir, "step3_fair_model_comparison.csv");
...
writetable(localizeFairComparisonTableForOutput(predictionResult.fairModelComparison), fairComparisonFile);
...
function comparisonTable = runFairModelComparison(cfg, featureData, selectedFeatures, targetName, lstmWithLagMetrics)
%RUNFAIRMODELCOMPARISON 在同一特征组和同一测试时间点上比较 LSTM 与 BP。
withLagFeatures = selectedFeatures;
withoutLagFeatures = selectedFeatures(~startsWith(selectedFeatures, "load_lag_"));
if isempty(withoutLagFeatures)
    withoutLagFeatures = selectedFeatures;
end

model = strings(4, 1);
featureSet = strings(4, 1);
usesLagFeatures = false(4, 1);
featureCount = zeros(4, 1);
RMSE = zeros(4, 1);
MAE = zeros(4, 1);
MAPE = zeros(4, 1);
R2 = zeros(4, 1);
...
row = 1;
[model, featureSet, usesLagFeatures, featureCount, RMSE, MAE, MAPE, R2, row] = ...
    appendFairComparisonRows(cfg, featureData, withLagFeatures, targetName, ...
    "with_lag", true, lstmWithLagMetrics, model, featureSet, usesLagFeatures, ...
    featureCount, RMSE, MAE, MAPE, R2, row);
[model, featureSet, usesLagFeatures, featureCount, RMSE, MAE, MAPE, R2, ~] = ...
    appendFairComparisonRows(cfg, featureData, withoutLagFeatures, targetName, ...
    "without_lag", false, [], model, featureSet, usesLagFeatures, ...
    featureCount, RMSE, MAE, MAPE, R2, row);

comparisonTable = table(model, featureSet, usesLagFeatures, featureCount, RMSE, MAE, MAPE, R2);
end

function [model, featureSet, usesLagFeatures, featureCount, RMSE, MAE, MAPE, R2, row] = ...
...
if isempty(existingLstmMetrics)
    lstmResult = trainLstmOrFallback(cfg, XSeq, ySeq, seqSplit);
    lstmMetrics = calculateMetrics(cfg, ySeq(seqSplit.test), lstmResult.yPredTest);
else
    lstmMetrics = existingLstmMetrics;
end

bpResult = trainBpOrFallback(cfg, alignedX, ySeq, seqSplit);
bpMetrics = calculateMetrics(cfg, ySeq(seqSplit.test), bpResult.yPredTest);

model(row) = "lstm_" + setName;
featureSet(row) = setName;
usesLagFeatures(row) = usesLag;
featureCount(row) = numel(featureNames);
RMSE(row) = lstmMetrics.RMSE;
MAE(row) = lstmMetrics.MAE;
MAPE(row) = lstmMetrics.MAPE;
R2(row) = lstmMetrics.R2;
row = row + 1;

model(row) = "bp_" + setName;
featureSet(row) = setName;
usesLagFeatures(row) = usesLag;
featureCount(row) = numel(featureNames);
RMSE(row) = bpMetrics.RMSE;
MAE(row) = bpMetrics.MAE;
MAPE(row) = bpMetrics.MAPE;
R2(row) = bpMetrics.R2;
row = row + 1;
end
...
function tableOut = localizeFairComparisonTableForOutput(tableIn)
%LOCALIZEFAIRCOMPARISONTABLEFOROUTPUT 生成公平输入对比表的中文导出副本。
tableOut = tableIn;
tableOut.model = localizeFairComparisonModelNames(tableOut.model);
tableOut.featureSet = localizeFairComparisonFeatureSets(tableOut.featureSet);
tableOut.Properties.VariableNames = {'模型', '特征组', '是否使用滞后特征', '特征数量', ...
    '均方根误差_kW', '平均绝对误差_kW', '平均绝对百分比误差_百分比', '决定系数R2'};
end
...
function namesOut = localizeFairComparisonModelNames(namesIn)
%LOCALIZEFAIRCOMPARISONMODELNAMES 将公平对比模型名转换为中文展示名。
namesIn = string(namesIn);
namesOut = strings(size(namesIn));
for i = 1:numel(namesIn)
    switch namesIn(i)
        case "lstm_with_lag"
            namesOut(i) = "含滞后特征的LSTM";
        case "bp_with_lag"
            namesOut(i) = "含滞后特征的BP";
        case "lstm_without_lag"
            namesOut(i) = "不含滞后特征的LSTM";
        case "bp_without_lag"
            namesOut(i) = "不含滞后特征的BP";
        otherwise
            namesOut(i) = namesIn(i);
    end
end
...
function namesOut = localizeFairComparisonFeatureSets(namesIn)
%LOCALIZEFAIRCOMPARISONFEATURESETS 将公平对比特征组名转换为中文展示名。
namesIn = string(namesIn);
namesOut = strings(size(namesIn));
for i = 1:numel(namesIn)
    switch namesIn(i)
